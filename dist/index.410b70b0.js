...
const isStageShown = (stage)=>{
    if ($(`cleo-stage-${stage}`)) return $(`cleo-stage-${stage}`).classList.contains("show");
    else return null;
};
const getStageShown = ()=>{
    ...
    // stage 2 is "hey user"
    for(let i = 0; i < 3; i++){
        if (isStageShown(i)) return i;
    }
    return -1;
};
const hideStage = (stage)=>{
    const stageNode = $(`cleo-stage-${stage}`);
    if (isStageShown(stage) && stageNode) stageNode.classList.remove("show");
};
const showStage = (stage)=>{
    const stageNode = $(`cleo-stage-${stage}`);
    if (!isStageShown(stage) && stageNode) stageNode.classList.add("show");
};
...
const handleStageSwitch = ()=>{
    ...
    if (currentStage === 0) hideInitialCTA();
...
//# sourceMappingURL=index.410b70b0.js.map
